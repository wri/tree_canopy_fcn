...
from utils.dataloader import HeightGreenDataset
...
LRS=[1e-3,1e-4]
NB_CATEGORIES=len(BUILTUP_CATEGORY_THRESHOLDS)+1
...
def loaders(**cfig):
    ...
    if (train_df.shape[0]>batch_size*8) and (valid_df.shape[0]>batch_size*2):
        ...
        dl_train=HeightGreenDataset.loader(
        ...
        dl_valid=HeightGreenDataset.loader(
            batch_size=batch_size,
            band_indices=['ndvi','ndwi'],
            dataframe=valid_df,
            means=MEANS,
